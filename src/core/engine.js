...
async function executeNodes(nodes) {
    ...
    try {
        for (const node of nodes) {
            console.log(`[Step ${stepCounter}/${totalSteps}] Processing ${node.nodeType}...`);
...
async function processNode(node) {
    switch (node.nodeType) {
        ...
        case 'Put':
            ...
            break;
        default:
            throw new Error(`Critical Error: Unknown AST node type: ${node.nodeType}`);
    }
}
...
async function processGet(node) {
    ...
    if (node.shouldUnpack) {
        if (node.unpackFormat && !downloadedFilePath.endsWith(`.${node.unpackFormat}`)) {
            throw new Error(`Critical Error: Expected format .${node.unpackFormat} but got something else for ${downloadedFilePath}`);
        }
        ...
        FileUtils.copyRecursiveSync(unpackDir, TMP_DIR, node.onlyPatterns, node.ignorePatterns);
        ...
        if (node.ignorePatterns.length > 0) {
            const shouldIgnore = node.ignorePatterns.some(pattern => minimatch(path.basename(destPath), pattern));
            if (shouldIgnore) {
...
async function processCreate(node) {
    ...
    if (node.contents === null) {
        ...
        if (node.options && node.options.type === 'ini' && typeof node.contents === 'object') {
...
async function processEdit(node) {
    ...
    if (!fs.existsSync(targetPath)) {
        throw new Error(`Critical Error: Cannot edit file ${node.targetPath}. File does not exist.`);
    }
...
async function processDelete(node) {
    ...
    if (fs.existsSync(targetPath)) {
        ...
        console.warn(`[Warning] Path to delete does not exist: ${node.targetPath}`);
    }
}
