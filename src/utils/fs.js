...
class FileUtils {
     ...
     */
    static copyRecursiveSync(source, dest, includePatterns = [], ignorePatterns = [], originalSource = null) {
        if (!fs.existsSync(source)) {
            throw new Error(`Critical Error: Source directory does not exist: ${source}`);
        }
        ...
        if (isDirectory) {
            ...
            fs.readdirSync(source).forEach((file) => {
                ...
                FileUtils.copyRecursiveSync(sourcePath, destPath, includePatterns, ignorePatterns, originalSource || source);
            });
        } else {
            ...
            if (includePatterns.length > 0) {
                ...
                const included = includePatterns.some(pattern => minimatch(relPath, pattern, { matchBase: true }));
                if (!included) return;
            }

            // Ignore logic for single file
            if (ignorePatterns.length > 0) {
                const ignored = ignorePatterns.some(pattern => minimatch(relPath, pattern, { matchBase: true }));
                if (ignored) return;
            }
    ...
    static unpackArchive(sourceFile, destDir) {
        if (sourceFile.endsWith('.zip')) {
            ...
            throw new Error(`Critical Error: Unsupported archive format for ${sourceFile}. Only .zip is currently supported.`);
        }
    }
}
