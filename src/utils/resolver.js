...
async function resolveUrl(source, options = {}) {
    ...
    const githubReleaseUrlRegex = /^https:\/\/github\.com\/([a-zA-Z0-9_.-]+)\/([a-zA-Z0-9_.-]+)\/releases\/latest$/;
    ...
    if (user && repo) {
        if (!options.artifactPattern) {
             throw new Error(`Critical Error: You must provide an artifact pattern (using .artifact()) when requesting a GitHub release.`);
        }
        ...
        try {
            ...
            throw new Error(`Critical Error: No artifact found matching '${options.artifactPattern}' in release ${response.data.tag_name} for ${user}/${repo}`);
        } catch (error) {
            if (error.response && error.response.status === 404) {
                throw new Error(`Critical Error: Repository or release not found: ${user}/${repo}`);
            }
            throw new Error(`Critical Error: Error resolving GitHub release: ${error.message}`);
        }
    }
